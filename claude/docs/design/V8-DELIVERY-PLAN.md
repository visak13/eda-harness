...
- 2026-08-24 C2-creative DONE: epic-2e4b479a0c (--banner, Sol consult chose the look, demo gate showed owner the banner, commit d7a6b4c, 12 tests) qa PASS. Fixes landed during the run: spawn registers explicit participant_id; checker auto-assign allowed when it is the doer of a ticket others check; cards — walk status before finish; coordinator no-hold rule. ALL C2 EPICS CLOSED (bug, rnd, creative).
- 2026-08-24 remainders fixed: board AUTO-ADVANCE (evidence complete -> in_review; all verdicts pass -> done, done-guards kept) — validated live via API, removes the walk-the-status race class; dependent-promotion dedupe (double ready event gone); pool spawns force ANSI color (FORCE_COLOR/CLICOLOR_FORCE) so shells render normally.
