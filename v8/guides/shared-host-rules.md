...
pool kills the fleet), starts the service with `start.ps1 -Only <svc>`, waits for its health route
(bounded, `-TimeoutSec`, default 60) and prints the new pid + rev. The pool, alone or in `all`,
lists the seats it takes offline and refuses without `-Force`; after a pool restart it prints each
seat's liveness. `update` refuses a dirty tree without `-Force`, pulls `--ff-only`, stops the
supervisor (it would restart what is being stopped), syncs deps, rebuilds the SPA when `v8/web/`
changed, restarts board → broker → pool (only with `-Force`) → mcp → bridge, then the supervisor.
