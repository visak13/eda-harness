...
| Monitor result latency / self-attach | not documented | the `Monitor` tool result is committed **260–317 ms** after invocation (20 calls, median ≈275 ms); events the script emits inside that window attach to the Monitor's OWN result | re-run 17:41:21/24/27Z: `queued_command` at +144 ms, result at +315 ms; Pi mirrors with `MONITOR_START_GRACE_MS=250` |
