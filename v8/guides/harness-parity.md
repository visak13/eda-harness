...
| Monitor shell on Windows | not documented | Claude runs Monitor commands under Git's bash; a pool-spawned Pi seat inherited a PATH where System32bash.exe (the WSL relay) won and its feed Monitor died at boot: `execvpe(/bin/bash) failed: No such file or directory` (engineer.s-174f83c926, task boh9ficen, 2026-09-17 13:53Z) — the first live GPT-driven ticket found it | Pi: `monitorShell()` = EDP_MONITOR_SHELL, else the first existing Git bash (ProgramFiles / LOCALAPPDATAPrograms), else PATH `bash`; fake harness "monitor shell never resolves to the WSL relay" |
