...
| one-shot early fire on :00/:30 | "up to 90 s early" | **measured 39.6 s early**: job 73602ab4 `30 18 17 9 *` (13:00:00Z) created from an IDLE seat fired at 12:59:20.355Z and auto-deleted (qa seat c7223cb9, scheduledFireId 78047a79…); earlier one-shots a40ab141/3e070b19 were due while busy and fired deferred at 15:57:31Z | Pi: `fnv1a(id) % 90` s early on :00/:30 — deterministic per job, the function [H]; second datum: 0a0c9d7d `0 19 17 9 *` (13:30:00Z) from THIS idle seat fired at 13:30:05.217Z (session 7edf0320) — 5 s LATE, not early; so "early" is not a fixed offset per slot: one job fires 39.6 s early, another 5 s after the minute. The per-job offset is real; its sign and function stay [H] |
| `durable` | "Has no effect" | **measured: no effect** — `durable:true` yields the identical result text (`Scheduled one-shot task 73602ab4 (30 18 17 9 *). Session-only … It will fire once then auto-delete.`) and the identical CronList line (`73602ab4 — 30 18 17 9 * (one-shot) [session-only]: …`); recurring probe 545f7928 (`17 4 1 1 *`, durable:true) likewise `Scheduled recurring job … Session-only … Auto-expires after 7 days.` | qa seat c7223cb9 13:00Z; this seat 13:01Z |
| 7-day expiry | "fire one final time, then are deleted" | NOT MEASURABLE on Claude in-session (a session-only job must live 7 days under one seat); Pi implements the documented rule and proves it under `EDP_PARITY_CLOCK_SCALE` (fake harness "7-day expiry": fires while alive, one final time, then gone) | [H] on the Claude side by construction; criterion c-8ef18b763b reworded accordingly |
...
| Monitor shell on Windows | not documented | Claude runs Monitor commands under Git's `bin\bash.exe` wrapper; a pool-spawned Pi seat inherited a PATH where System32\bash.exe (the WSL relay) won and its feed Monitor died at boot: `execvpe(/bin/bash) failed: No such file or directory` (engineer.s-174f83c926, task boh9ficen, 13:53Z); the next fix picked `usr\bin\bash.exe`, which launched directly under a Windows PATH has no coreutils: `sleep: command not found` (qa.s-174f83c926, task z6tapf7pa, 14:12Z) — both found by the first GPT-driven ticket, 2026-09-17 | Pi: `monitorShell()` = EDP_MONITOR_SHELL, else Git's `bin\bash.exe` wrapper (puts /usr/bin on PATH), else `usr\bin\bash.exe`, else PATH `bash`; fake harness "monitor shell never resolves to the WSL relay" + "is the Git wrapper bash" |
...
| recurring jitter formula | "10 % (max 15 min)" | measured 628–629 s on b2c55175; `fnv1a(id) % 900` gives 631 s — the hash is NOT Claude's (qa A10). Per-job constant offset is the measured fact; the function stays [H] | Pi: deterministic per job, rescheduled from the unjittered slot so the PERIOD is exact (qa A10 fix) **5-min period measured**: job 334a85c4 `*/5 * * * *` created 14:19:0xZ on this Claude seat (idle) fired at 14:23:50.627Z for the 14:20 slot = 230 s late — far above the documented "10 % of the period" (30 s), so the offset is NOT period-scaled; it is a per-job constant in [0, 15 min) whatever the period. The GPT qa seat's */5 job b11badb4 carries 660 s (Pi: fnv1a % 900) — same rule, different hash [H]. SECOND fire of 334a85c4 at 14:28:35.911Z = 215 s after the 14:25 slot (285 s after the first fire): the offset is NOT even constant per job (230 → 215 s), so the [H] stays on the function; the job was then CronDeleted |
| rate-limit steady state | "too many events are automatically stopped" | Claude: 2 delivered then `[6 events suppressed …]` per ~0.8 s at 10 lines/s; Pi's continuous token bucket (2.5/s) reported 1 delivered / [1–2 suppressed] live (GPT qa drill task trh9q8eh4, 14:42Z, c-6dbfeaf428) — replaced by a DISCRETE window: burst 20, +2 tokens per 800 ms, so 10 lines/s gives exactly 2 delivered then `[6 events suppressed …]`; lines suppressed after the last delivered one are announced at stream end [H] | fake harness "rate gate: ~22 pass first, then 2 delivered / [6 suppressed] per window" (virtual clock, 150 lines) |
