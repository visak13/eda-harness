...
function OtherInstance($p) {
  ...
  # the server's pipe is named for its port (start-code.ps1: \\.\pipe\edp-code-<port>-<rand>)
  if ($p.CommandLine -match '--socket\s+"?\\\\\.\\pipe\\edp-code-(\d+)-') { return ([int]$Matches[1] -ne $PORT) }
