...
- knowledge.py _always_include selects binding and must-follow decisions regardless of the question. The packing loop limits their text-only entries to ALWAYS_MAX_BYTES=2000; overflow IDs appear in binding_trimmed and mandatory_overflow. Therefore the design's absolute “never cut” statement would be inaccurate for this code.
...
- MAX_BYTES=8000 counts serialized entries. Lessons have up to 1500 bytes and three entries within that total. Answers and optional source excerpts use remaining space. The full response also includes body text and a receipt; 8000 is not the size of the whole response envelope.
