...
def flow():
    ...
    c.text(449,302,'Always include live binding\nand must-follow decisions.\nText only: up to 2,000 bytes.',27)
    c.box((443,424,822,493),'#FFF1DA','#FFF1DA',9)
    c.text(454,436,'If they overflow, cut extras\nand report their IDs.',26,True,AMBER)
    ...
    c.text(585,605,'6  Fit the pack to 8,000 bytes',35,True,BLUE)
    ...
    c.text(610,733,'2,000',30,True,BLUE)
    c.text(805,733,'4,500 remaining',29,True)
    c.text(1185,733,'1,500',29,True,TEAL)
    c.text(585,817,'Rules: up to 2,000 bytes (about 2 KB).',27,True,BLUE)
    ...
    c.text(84,670,'≈6 records',49,True)
    ...
    c.text(1464,936,'Counts by type + record IDs\nRules cut if the allowance overflowed\nRead a cut record by its ID',26)
    c.line([(1360,950),(1440,950)],AMBER)
    c.text(60,1132,'Current code: binding overflow is reported. The design’s “never cut” promise is not absolute.',27,True,AMBER)
