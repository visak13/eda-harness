...
export class ThumbWorker {
  ...
  /** each job with the worker it was posted to: a dying worker fails only its own jobs */
  private waiting = new Map<number, { w: Worker; done: (t: Thumb) => void }>();
  ...
  private start(): Worker {
    ...
    w.on('message', (m: { id: number; thumb: Thumb }) => { this.waiting.get(m.id)?.done(m.thumb); this.waiting.delete(m.id); });
    const fail = (why: string) => {
      if (this.worker === w) this.worker = undefined;
      for (const [id, j] of this.waiting) if (j.w === w) { this.waiting.delete(id); j.done({ ok: false, reason: why, transient: true }); }
    };
  ...
  thumb(bytes: Uint8Array, type: string): Promise<Thumb> {
    const job = this.chain.then(() => new Promise<Thumb>(resolve => {
      let w: Worker;
      try { w = this.start(); } catch (e) { resolve({ ok: false, reason: `no thumbnail worker (${(e as Error)?.name ?? 'error'})`, transient: true }); return; }
      const id = ++this.seq;
      const timer = setTimeout(() => {
        if (!this.waiting.has(id)) return;
        this.log('thumbs: job timed out; restarting the worker');
        this.waiting.delete(id);
        resolve({ ok: false, reason: 'thumbnail timed out', transient: true });
        if (this.worker === w) this.worker = undefined; // the next job starts a fresh worker, never posts to this one
        void w.terminate();
      }, JOB_TIMEOUT_MS);
      this.waiting.set(id, { w, done: t => { clearTimeout(timer); resolve(t); } });
