...
export const PAGES: Record<string, PageCopy> = {
  ...
  epic: {
    ...
    items: [
      ...
      { key: "steer", label: "Steer this epic", text: "posts a steer on the epic thread. Wakes: the architect always, plus the seat you address.", control: true },
      ...
      { key: "change-status", label: "Change status", text: "the moves the board allows from the current phase, with what each means. Wakes on ready/in_review/done/blocked: the owner and the architect; on a story, also its assignee.", control: true },
      ...
      { key: "spawn-architect", label: "Spawn the architect", text: "starts a fresh architect shell for this epic (architect.<epic>) through the pool; shown only when the pool can spawn; the board's answer is shown verbatim.", control: true },
