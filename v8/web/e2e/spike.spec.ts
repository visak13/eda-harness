...
test("shell renders identity + whoami and receives a live feed event within 5s", async ({ page }) => {
  ...
  // Seam 3: post a note (architect → owner) and watch the live pill appear over the feed. The
  // shell holds feed events as "N new · refresh" ONLY while a composer is dirty (§16.1 listening
  // contract; a clean page refreshes silently) — so start a conversation draft first.
