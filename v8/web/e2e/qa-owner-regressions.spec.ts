...
test('status tooltip stays near its trigger instead of viewport bottom-left',async({page})=>{
 ...
 // the phone summary badge is first in DOM but hidden on desktop
 const status=page.getByText('Drafted',{exact:true}).filter({visible:true}).first();await status.hover();
